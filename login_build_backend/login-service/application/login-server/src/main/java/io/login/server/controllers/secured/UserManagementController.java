...
public class UserManagementController {
    ...
    @PutMapping("/update-user-profile-details")
    ...
    public ResponseEntity<Boolean> changeUserPasswordOnRequest(@RequestBody ChangeUserPassword changeUserPassword,
                                                            @PathVariable("user") String userName) {
        UserAccount userAccount = this.userService.getLoginUser(userName);
        Boolean isPasswordChanged = false;
        if (userAccount.getPassword().equals(changeUserPassword.getOldPassword())) {
            isPasswordChanged =
                    this.userService.changePasswordOnUserRequest(changeUserPassword.getNewPassword(), userName);
        } else {
            ErrorModel errorModel = new ErrorModel();
            errorModel.setHttpStatusCode(401);
            errorModel.setApplicationErrorCode(12341);
            errorModel.setUserInterfaceMessage("Incorrect Old Password !");
